...
WebSQL Database jest mechanizmem, który do przechowywania, odczytu oraz aktualizacji danych po stronie klienta wykorzystuje strukturalny język zapytań SQL. Metoda działania mechanizmu bazuje na specyfikacji bazy SQLite. Mechanizm WebSQL posiada zarówno zalety przypisywane relacynym bazom danych oraz kilka zalet cechujących metody lokalnego zapisu danych:
...
Część serwerowa została zaimplementowana w języku PHP (\emph{PHP Hypertext Preprocessor}) w wersji 5.5 z wykorzystaniem frameworka MVC (\emph{Model View Controller}) CodeIgniter. Relacyjna baza danych wykorzystana w projekcie to MySQL.
